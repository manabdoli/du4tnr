...
## Package and Website with `pkgdown`
- Use `main` as the deployment source
- Use 'pkgdown::build_site` to generate and deploy the wibsite to `gh_pages` branch on GitHub.
  - No need for a local branch
- Use a `features/aDev` branch for developing new features before adding it back to the deployment branch. 
